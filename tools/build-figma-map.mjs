...
/* Half the components compose type from primitives (`fontWeight` + `fontSize` inline) instead of
   using one of the 14 roles. Measured 21 Sep 2026: 80 declarations across 38 components. Thirty of
   them match a role EXACTLY on weight, size and leading — those attach to the role in Figma with
   zero visual change. The other fifty match nothing, and the owner's ruling (21 Sep) is to build
   them faithfully and register them here rather than invent styles or normalise the code.

   So Gate A's type rule is: a text node uses a role, OR its combination appears below. Anything
   else is a literal and a failure. This file is what makes that enforceable — and generated, so
   the register cannot quietly grow. */
/* Tracking is part of the combination. The eyebrow role carries 0.08em and the label role 0.04em
   (measured: --tracking-eyebrow appears only beside --type-eyebrow-font, --tracking-pill only
   beside --type-label-font). Ignoring it matched Badge's bold/10 at 0.04em to `eyebrow` at 0.08em,
   which Gate B would then have failed for a reason the register had hidden. */
const ROLE_TRACK = { eyebrow: 0.08, label: 0.04 };
const ROLE_TRIPLE = new Map(typeRoles.map((t) => {
  const n = (css) => { const d = byName.get(css); return d ? parseFloat(d.value) : null; };
  return [t.role, { w: n(t.weight), s: n(t.size), l: t.leading === '1' ? null : n(t.leading),
    t: ROLE_TRACK[t.role] ?? 0 }];
}));

const inlineType = new Map();               // "w/s/l" → { w, s, l, uses, components:Set }
const TNUM = (css) => { const d = byName.get(css); return d ? parseFloat(d.value) : null; };
for (const p of sources) {
  const src = await readFile(p, 'utf8');
  const comp = basename(p, '.jsx');
  for (const m of src.matchAll(/\{[^{}]*fontSize:[^{}]*\}/g)) {
    const blk = m[0];
    const w = blk.match(/fontWeight:\s*'?var\((--weight-[a-z]+)\)/);
    const z = blk.match(/fontSize:\s*'?var\((--(?:text|display)-[0-9-]+)\)/);
    const l = blk.match(/lineHeight:\s*'?var\((--leading-[0-9]+)\)/);
    if (!z) continue;
    /* letter-spacing is part of the combination, from a token or written as a literal em — three
       components write 0.08em / 0.04em by hand, which is F-81's third paragraph. Leaving it out
       matched Badge's bold/10 at 0.04em to the eyebrow role at 0.08em, and Gate B would then have
       failed for a difference the register had hidden. */
    const tk = blk.match(/letterSpacing:\s*'?var\((--tracking-[a-z]+)\)/);
    const tl = blk.match(/letterSpacing:\s*'([0-9.]+)em'/);
    const track = tk ? parseFloat(byName.get(tk[1]).value) : tl ? parseFloat(tl[1]) : 0;
    const key = [w ? TNUM(w[1]) : null, TNUM(z[1]), l ? TNUM(l[1]) : null, track].join('/');
    if (!inlineType.has(key)) inlineType.set(key, { w: w ? TNUM(w[1]) : null, s: TNUM(z[1]), l: l ? TNUM(l[1]) : null, t: track, uses: 0, components: new Set() });
    const e = inlineType.get(key);
    e.uses++; e.components.add(comp);
  }
}
const roleFor = (e) => {
  for (const [role, r] of ROLE_TRIPLE) {
    if (e.w === r.w && e.s === r.s && (e.l === null || r.l === null || e.l === r.l) && e.t === r.t) return role;
  }
  return null;
};
const typed = [...inlineType.values()].map((e) => ({ ...e, role: roleFor(e) }))
  .sort((a, b) => b.uses - a.uses);
const matched = typed.filter((t) => t.role);
const gaps = typed.filter((t) => !t.role);
const totalUses = typed.reduce((n, t) => n + t.uses, 0);
const allComps = new Set(typed.flatMap((t) => [...t.components]));

const reg = `<!-- GENERATED by tools/build-figma-map.mjs. Do not edit. Run \`npm run build:figma-map\`. -->

# The type register

The 14 roles are the ramp. **${totalUses} declarations across ${allComps.size} components do not use
...
## Attach the role — ${matched.reduce((n, t) => n + t.uses, 0)} uses, ${matched.length} combinations
...
| --- | --- | --- | --- | --- | --- | --- |
${matched.map((t) => `| ${t.w ?? '—'} | ${t.s} | ${t.l ?? '—'} | ${t.t ? t.t + 'em' : '—'} | \`${t.role}\` | ${t.uses} | ${[...t.components].sort().join(', ')} |`).join('\n')}

## No role — ${gaps.reduce((n, t) => n + t.uses, 0)} uses, ${gaps.length} combinations

Built faithfully in Figma with explicit values, by the owner's ruling of 21 Sep: inventing ${gaps.length}
...
| --- | --- | --- | --- | --- | --- |
${gaps.map((t) => `| ${t.w ?? '—'} | ${t.s} | ${t.l ?? '—'} | ${t.t ? t.t + 'em' : '—'} | ${t.uses} | ${[...t.components].sort().join(', ')} |`).join('\n')}
...
`;
await writeFile(join(OUT, 'type-register.md'), reg);
...
for (const p of sources) {
  ...
  try {
    ...
    /* The props interface: single-line or multi-line, with or without an extends clause.
       Two earlier bugs here — the template literal was over-escaped (\\\\s reached the RegExp as a
       literal backslash-s, so the named match NEVER fired and every component fell through to the
       generic one), and neither pattern allowed `SurfaceProps extends React.HTMLAttributes<…>`,
       which is why Surface reported no props at all. */
    const named = `(?:interface|type)\\s+${name}Props(?:\\s+extends\\s+[^{]+)?\\s*=?\\s*\\{([\\s\\S]*?)\\n?\\}\\s*(?:;|\\n)`;
    const iface = dts.match(new RegExp(named))
      || dts.match(/(?:interface|type)\s+\w*Props(?:\s+extends\s+[^{]+)?\s*=?\s*\{([\s\S]*?)\n?\}\s*(?:;|\n)/);
    if (iface) {
...
console.log(`type-register.md: ${totalUses} inline declarations in ${allComps.size} components — ${matched.length} combinations match a role, ${gaps.length} do not`);
