...
| 8 ✅ | **Journey D — Amit's proposal** — built 19 Sep as `screens/journey-d/proposal.html` | Home promised it in a row and a starter and it went nowhere. It is also `ResultCard`'s first screen: the component shipped in v9 and sat on no screen because the three journeys it exists for were unbuilt | `ResultCard`, `ResultActions`, `ResultPrimary`, `VersionRow`, `MoneyComposer` |
| 9 ✅ | **Journey E — the rebalance, reached cold** — built 19 Sep as `screens/journey-e/rebalance.html` | Journey B rebalances Sharma at the end of a drift explanation. An advisor who asks for one directly had no answer at all — and "rebalance" is not an instruction until someone says how far | `ResultCard journey='rebalance'`, `ResultPrimary moves`, and `StepTurn`'s `extra` slot |
| 10 ✅ | **Journey F — Meera's client review** — built 19 Sep as `screens/journey-f/review.html` | The last of the three journeys `ResultCard` was built for. Its one question is not about facts — the book has them all — it is what the review is FOR, because the same holdings make a different document for a file, a meeting and a decision | `StatTile locked` (on no screen since v9), `ConcentrationBar`, `LiveRail onAnswer` |
