      ...
      <div style={{ display: 'flex', alignItems: 'stretch' }}>
        <Pressable
          ...
          style={{
            ...
            padding: 'var(--space-14)', textAlign: 'left', background: 'transparent',
            cursor: hasPage ? undefined : 'default',
          }}>
          {/* ROW 1 — who it is. The AMC mark is a letter in a disc, not a logo image: this system
              ships no images, and a 24px remote logo is a request that can fail on a card. */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-10)', width: '100%' }}>
            ...
            {badge && !badgeInline && <span style={{ flex: '0 0 auto' }}><Badge variant="status" tone={badgeTone}>{badge}</Badge></span>}
          </div>
          ...
          {/* ROW 2 — the one number, and the path that produced it. */}
          {(headline || spark) && (
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 'var(--space-12)', width: '100%' }}>
              {headline && (
                <span style={{ display: 'flex', flexDirection: 'column', gap: 0, minWidth: 0 }}>
                  ...
                  <span style={{ font: 'var(--type-caption-font)', color: 'var(--color-muted)' }}>{headline.label}</span>
                </span>
              )}
              {spark && spark.length > 1 && (
                <span style={{ flex: '0 0 auto', paddingBottom: 2 }}>
                  <ChartSpark points={spark} width={84} height={26} />
                </span>
              )}
            </div>
          )}
              ...
              {facts.map((f, i) => (
                <React.Fragment key={f.label}>
                  {/* The middot is its OWN flex child, not a prefix inside the next pair. Nested, it
                      ...
                      separates reads as belonging to one of them. */}
                  {i > 0 && <span aria-hidden="true" style={{ color: 'var(--color-line)' }}>·</span>}
