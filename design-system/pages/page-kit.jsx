    ...
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '28px 24px 64px', display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      {missing.length ? (
        <div style={{ borderRadius: 12, background: 'var(--color-status-over-bg)', padding: 16 }}>
          ...
          <p style={{ ...note, color: 'var(--color-ink-soft)' }}>This page needs {missing.map((n, i) => <React.Fragment key={n}>{i ? ', ' : ''}<code>{n}</code></React.Fragment>)}, which <code>_ds_bundle.js</code> does not carry yet. The bundle recompiles from source at the end of a turn — reload then. The page is not broken; the build is behind it.</p>
        </div>
